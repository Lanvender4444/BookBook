...
function generateCover(title, outputPath, width = 400, height = 560) {
  const canvas = createCanvas(width, height)
  drawBauhausCover(canvas, title, width, height)
...
function main() {
  ...
  if (args.length < 2) {
    console.log('Usage: node generate-covers.js <input-file> <output-dir>')
    console.log('Input file should contain book titles, one per line')
    process.exit(1)
  }
  ...
  console.log(`Found ${titles.length} titles to generate covers for`)

  titles.forEach((title, index) => {
    const filename = `cover-${index + 1}.png`
    const outputPath = path.join(outputDir, filename)
    generateCover(title, outputPath)
  })
...
export { generateCover, drawBauhausCover }
