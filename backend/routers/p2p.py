...
async def connect_to_peer(req: ConnectRequest):
    ...
    if result.get("status") == "error":
        raise HTTPException(
            status_code=400, detail=result.get("message", "Connection failed")
        )
    return result


@router.post("/{peer_host}/books")
async def fetch_peer_books(peer_host: str, port: int = P2P_PORT):
...
async def download_book_from_peer(peer_host: str, book_id: str, port: int = P2P_PORT):
    book_data = await p2p_service.fetch_book(peer_host, book_id, port)
    if book_data is None:
        raise HTTPException(
            status_code=400, detail="Failed to connect to peer or book not found"
        )
    if not book_data:
...
def create_share_token(req: ShareRequest):
    result = p2p_service.create_share_token(
        book_id=req.book_id, expires_hours=req.expires_hours
    )
    return result
        ...
        if result.get("all_books") and "books" in result:
            ...
            for meta in books_meta:
                try:
                    ...
                    book_data = None
                    for host in hosts_to_try:
                        book_data = await p2p_service.fetch_book(
                            host, meta["id"], req.port
                        )
                        if book_data:
                            break
                    if book_data:
                        saved_id = save_p2p_book(
                            db, book_data, book_data.get("author_id", "p2p")
                        )
                        saved_ids.append(saved_id)
                except Exception as e:
            ...
            if not saved_ids:
                raise HTTPException(
                    status_code=404, detail="No books could be downloaded"
                )
...
@router.post("/sync")
async def sync_from_peer(peer_host: str = None, port: int = P2P_PORT):
    if not peer_host:
