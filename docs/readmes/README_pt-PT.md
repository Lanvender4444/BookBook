...
O backend irá funcionar em http://localhost:8000
...
npm run dev
```
O frontend irá funcionar em http://localhost:5173
...
## Implementação com Docker (Opcional)

Se utilizar Docker, certifique-se de que o Docker Desktop está instalado, e depois execute:

```bash
docker-compose up -d
```

Isto irá iniciar tanto o frontend como o backend.
...
Se utilizar funcionalidades P2P, certifique-se de que as portas 47832 (UDP) e 47833 (TCP) estão abertas na sua LAN.
