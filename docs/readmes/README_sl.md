...
## Skripte za Hiter Začetek (Windows)

### Zagon Backend z Enim Klikom

Ustvarite `start_backend.bat`:
```batch
@echo off
cd backend
uv run uvicorn main:app --reload --port 8000
...
### Zagon Frontend z Enim Klikom

Ustvarite `start_frontend.bat`:
```batch
@echo off
cd frontend
npm run dev
...
### Zagon Vseh Storitev z Enim Klikom

Ustvarite `start_all.bat`:
```batch
@echo off
